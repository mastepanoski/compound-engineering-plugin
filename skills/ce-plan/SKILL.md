...
- Do not write a launch prompt into the doc. The launch prompt is generated at handoff (Phase 5.4 menu — `/goal` copy-paste on Claude Code, `create_goal` on Codex) from the plan's current content, so it never goes stale; it points to Reader Index, Goal Capsule, Verification Contract, Definition of Done, and U-IDs rather than duplicating them.
