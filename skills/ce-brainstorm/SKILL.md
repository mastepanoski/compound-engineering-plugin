...
Write to `docs/plans/YYYY-MM-DD-NNN-<type>-<topic>-plan.<md|html>` — extension follows `OUTPUT_FORMAT`. Include `artifact_contract: ce-unified-plan/v1`, `artifact_readiness: requirements-only`, and `product_contract_source: ce-brainstorm`. Title is `<Name> - Plan` (matching the H1; no conventional-commit prefix). Keep the doc light and standalone-readable: a Goal Launch Block holding a single `Next step: run /ce-plan <path>` line, a Goal Capsule (objective, product authority, open blockers — no stop-condition prose), and the Product Contract. Do **not** emit a Reader Index or the "do not run ce-work/goal" lecture for requirements-only — the `artifact_readiness` frontmatter carries that for agents; restating it is process exhaust. See `references/brainstorm-sections.md`. Confirm with the absolute path so the reference is clickable.
